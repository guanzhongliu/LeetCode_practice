...
策略：Kruskal最小生成树，进阶可以用树状数组（暂未学习）
'''
...
class Solution_Prim(object):
    def minCostConnectPoints(self, points: List[List[int]]) -> int:
        ...
        total_cost= 0
        nodes.add(0)
        node_pre, min_len = 0, float('inf')
        for _ in range(num - 1):
            for node in nodes:
                for edge in edges[node]:
                    if edge[0] < min_len and edge[1] not in nodes:
                        min_len = edge[0]
                        node_pre = edge[1]
            total_cost += min_len
            nodes.add(node_pre)
